...
def main(args):
    try:
        ...
        dataset = dataset.map(
            lambda x: update_text(x, base_prompt, tokenizer, args.label)
        )
        ...
        while len(dataset) > curr_idx + args.batch_size:
            ...
            batch_results, count = process_batch(
                llm, curr_batch, sampling_params, args.label
            )
            count_f += count
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tensor_parallel_size", type=int, default=2, help="Number of GPUs to use."
    )
    parser.add_argument("--batch_size", type=int, default=100, help="Batch size.")
    parser.add_argument(
        "--max_model_len", type=int, default=32000, help="Max sequence length."
    )
    parser.add_argument(
        "--label", type=str, required=True, help="Label to insert in the prompt."
    )
    parser.add_argument(
        "--limit", type=int, default=750000, help="Limit of examples to process."
    )
