...
def get_labels(sample, model_path):
    ...
    label = model.predict(sample['text_formatted'].replace('\n', ' '),threshold=0.98,k=-1)
...
def process_arrow(file_path,model_path,cache_dir):
    ...
    shard_dataset = load_dataset("arrow", data_files=[file_path],cache_dir=cache_dir)["train"]
    
    shard_dataset = shard_dataset.map(
        lambda sample: {"text": sample["text"],"text_formatted": preprocess_text(sample["text"])},
        num_proc=100,
