...
def main(args):
    ...
    for val in range(len(data)):
        ...
        if args.label == label:
            combined_data_list.append(
                {"text": text_dataset[val]["text"][:4000], "Label": args.label}
            )
        else:
            combined_antidata_list.append(
                {"text": text_dataset[val]["text"][:4000], "Label": "Not_" + args.label}
            )

    data_label = Dataset.from_list(combined_data_list)
    size = len(data_label)

    antidata = Dataset.from_list(combined_antidata_list).select(
        range(min(len(combined_antidata_list), size))
    )

    data_neutra = (
        Dataset.load_from_disk(args.neutral_path)
        .shuffle()
        .select(range(size - len(antidata)))
    )
    data_neutra = data_neutra.map(
        lambda x: {"text": x["texto"], "Label": "Not_" + args.label}
    )
    ...
    os.system(
        f"head -n {int(size*2 * 0.8)} {args.model_path}/data.txt > "
        f"{args.model_path}/data.train"
    )
    os.system(
        f"tail -n {int(size*2 * 0.2)} {args.model_path}/data.txt > "
        f"{args.model_path}/data.valid"
    )

    model = fasttext.train_supervised(
        input=f"{args.model_path}/data.train", lr=0.1, epoch=25, wordNgrams=2
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--label", type=str, required=True, help="Label to train the model on"
    )
